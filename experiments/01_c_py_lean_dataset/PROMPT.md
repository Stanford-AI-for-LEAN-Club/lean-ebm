...
- Lean translation of the C and Python code
- Test cases of the same function in C and Python and the same test cases in Lean
- Theorem statements in Lean that prove major properties of the behavior of the C and Python code
- Theorem proofs in Lean about those above statements
...
- signed overflow is UB in C → document assumption that inputs avoid it, OR model as `Int32`/`Int64` and add a precondition `h : x ∈ Set.Icc Int32.min Int32.max`
...
- Functions that are only correct up to rounding → state theorems as: `|lean_result - exact_result| ≤ n * Float.epsilon`
- `fma`, `sqrt`, `ceil`, `floor`, `round` → use Lean's `Float.sqrt` etc. or axiomatize
- Do NOT attempt to prove termination/correctness of iterative float algorithms (e.g., Newton-Raphson) without a convergence bound as a precondition
...
- Null-terminated strings → model as `Array UInt8` with a terminator proof: `∃ i, i < arr.size ∧ arr[i] = 0 ∧ ∀ j < i, arr[j] ≠ 0`
...
- Unions → SKIP by default; or model the active variant as an `inductive` with one constructor per member, adding a note that this loses aliasing semantics
